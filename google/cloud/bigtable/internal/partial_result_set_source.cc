...
#include "google/cloud/bigtable/internal/merge_chunk.h"
...
StatusOr<bigtable::QueryRow> PartialResultSetSource::NextRow() {
  ...
  values.reserve(metadata_->proto_schema().columns_size());
  for (auto const& column : metadata_->proto_schema().columns()) {
    values.push_back(
        bigtable_internal::FromProto(column.type(), std::move(*value_it)));
    ++value_it;
  }
